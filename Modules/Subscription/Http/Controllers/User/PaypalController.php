...
class PaypalController extends Controller
{
    public function payWithPaypal(Request $request, $slug){
        ...
        $provider_paypal_setting = ProviderPaypal::where('provider_id', $influencer_id)->first();
        ...
        if($paypal_setting->account_mode == 'sandbox'){
            config(['paypal.sandbox.client_id' => $provider_paypal_setting->client_id]);
            config(['paypal.sandbox.client_secret' => $provider_paypal_setting->secret_id]);
        }else{
            config(['paypal.sandbox.client_id' => $provider_paypal_setting->client_id]);
            config(['paypal.sandbox.client_secret' => $provider_paypal_setting->secret_id]);
        ...
        $response = $provider->createOrder([
            "intent" => "CAPTURE",
            "application_context" => [
                "return_url" => route('provider-paypal-payment-success'),
                "cancel_url" => route('provider-paypal-payment-cancled'),
            ],
            "purchase_units" => [
                0 => [
                    "amount" => [
